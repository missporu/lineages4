...
$result = mysql_result(mysql_query("SELECT COUNT(*) FROM `msg_r` WHERE user_from = '$nick'"),0);
if ($result == 0)
{
...
$puslap = floor($viso/5);
$message = mysql_query("SELECT * FROM msg_r WHERE user_from = '$nick'  ORDER BY id DESC LIMIT $num,5");
while($msg = mysql_fetch_array($message))
{
if ($msg[read] == 1)
{
//mysql_query("UPDATE `msg_r` SET `read` = 0 WHERE `user_from` = '$nick'"); убераем что бы не пропало не прочитаное
}
if ($msg[read] == 1)
{
...
echo "<br/><b><font color=grey>Кому:</font> </b><b><a href='/search.php?nick=".$from."&amp;go=go'>$from</a><font color=#686868></b> <small>$msg[time]</small></font>
<br/> $read 
...
if ($_GET[page] > 0)
{
echo "<a href=\"my_msg.php?nick=$nick&amp;page=$back\">back</a>";
}
elseif ($_GET[page] == 0)
{
echo "back";
...
{echo "<a href=\"my_msg.php?nick=$nick&amp;page=$next\">next</a><br/>";}
else
{echo "next<br/>";}
}
