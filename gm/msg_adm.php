...
echo "<p><font color=orabge>Рассылка сообщений администрации</font></p><hr>";
...
echo '
<select name=\"type\">
<option value=\"5\">Основателям</option>
<option value=\"4\">Админам</option>
<option value=\"3\">Ст.модераторам</option>
<option value=\"2\">Модераторам</option>
...
$usrreg = mysql_query("SELECT * FROM `users` WHERE `dostup` > 0");
   While($usr = mysql_fetch_array($usrreg)){
mysql_query("INSERT INTO `msg_r` SET `user_from` = 'Рассылка', `user_to` = '$usr[usr]', `time` = '$time', `read` = 1, `mail_msg` = '$text'"); // отправляем сообщение
}
