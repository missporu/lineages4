...
$req = mysql_query("SELECT * FROM `res` WHERE `lat_name` = 'VIP 1000' LIMIT 1");
...
mysql_query("UPDATE `res` SET `kol` = `kol` -1 WHERE `usr` = '$log' and `lat_name` = 'VIP 1000'");
$req22 = mysql_query("SELECT * FROM `res` WHERE `usr` = '$log' and `lat_name` = 'VIP 1000'");
$res=mysql_num_rows($req22);  
$rs = mysql_fetch_array($req22);
if($nk <= 0){ 
mysql_query("DELETE FROM `res` WHERE `usr` = '$log' and `lat_name` = 'VIP 1000' LIMIT 1");
...
`tip` = '1000',
`time` = '$times'");
echo' VIP x1000 активирован на 30 дней!';
